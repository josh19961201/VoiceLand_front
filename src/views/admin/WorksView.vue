<template>
  ...
  <n-modal v-model:show="form.dialog" preset="dialog" title="Dialog" negative-text="取消" positive-text="送出" :mask-closable="false" :on-negative-click="closeForm" :on-positive-click="submitForm" :closable="false" :loading="loading">
    <template #header>
      <div>{{ form._id.length > 0 ? '編輯作品' : '新增作品' }}</div>
    </template>
    <n-card>
      <n-form ref="fromRef" :rules="rules" :model="form">
        <n-grid :cols="12" :x-gap="12">
          <n-form-item-gi :span="10" label="作品名稱" path="title">
            <n-input v-model:value="form.title" placeholder="請輸入作品名稱" />
          </n-form-item-gi>
          <n-form-item-gi :span="2" label="公開" path="display">
            <n-switch v-model:value="form.display" />
          </n-form-item-gi>
          <n-form-item-gi :span="6" label="作品日期" path="date">
            <n-date-picker
          ...
          <n-form-item-gi :span="12" label="演出者" path="artist">
            <n-select v-model:value="form.artist" :options="artist" :show-checkmark="false" />
          </n-form-item-gi>
          <n-form-item-gi :span="12" label="作品資訊" path="description">
            <n-input
              v-model:value="form.description" type="textarea" :autosize="{ minRows: 3,maxRows: 5 } " placeholder="說說這個作品有多酷！"
            />
          </n-form-item-gi>
  ...
  <n-divider />
  <n-data-table :columns="columns" :data="works" />
</template>
...
const openDialog = (idx) => {
  if (idx === -1) {
    ...
    form._id = works[idx]._id
    // form.image = works[idx].image
    form.title = works[idx].title
    form.date = works[idx].date
    form.description = works[idx].description
    form.display = works[idx].display
    form.artist = works[idx].artist
    form.links = works[idx].links
    form.dialog = works[idx].dialog
    form.idx = idx
    ...
    if (!errors) {
      ...
      try {
        if (form._id.length === 0) {
          const { data } = await apiAuth.post('/works', fd)
          ...
          const { data } = await apiAuth.patch('/works/' + form._id, fd)
          works[form.idx] = data.result
          dialog.success({
            title: '編輯成功',
            content: '作品資訊編輯成功',
            positiveText: '確認'
...
(async () => {
  try {
    const { data } = await apiAuth.get('/works/all')
    works.push(...data.result)
  } catch (error) {
    dialog.error({
      title: '載入失敗',
